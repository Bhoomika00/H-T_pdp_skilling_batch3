...
public class Food_order_app {
	...
	public static void register() {
		...
		String sql = "insert into food_register(email_id,password) values (?,?)";
	...
	public static void login() {
		...
		try (Connection conn=DriverManager.getConnection(DB_URL,DB_user,DB_pass);
				 PreparedStatement ps=conn.prepareStatement("select * from food_register where email_id=? and password=?;");)
			 {
		...
		while(true) {
			...
			case 1:System.out.println("----Menu-----");
				System.out.println("1.PAV BHAJI   RS.120");
				   ...
				   //System.out.println("5.HOT COFFEE   RS.30");
	...
	private static void total(String emailid) {
		// TODO Auto-generated method stub
		String sql = "select sum(price) as a from food_order where email_id =?";
		try(Connection conn = DriverManager.getConnection(DB_URL, DB_user, DB_pass);
				PreparedStatement ps = conn.prepareStatement(sql)){
			ps.setString(1,emailid);
	...
	private static void cancel_order(String emailid, int cancel_no) {
		// TODO Auto-generated method stub
		String sql = "delete from food_order where email_id = ? and item = ?)";
		try(Connection conn = DriverManager.getConnection(DB_URL, DB_user, DB_pass);
				PreparedStatement ps = conn.prepareStatement(sql)){
			ps.setString(1, emailid);
			if(cancel_no == 1) 
	...
	public static void place_order(String email_id,int order_no) {
		...
		String sql = "insert into food_order(email_id,item, price) values (?,?,?)";
		try(Connection conn = DriverManager.getConnection(DB_URL, DB_user, DB_pass);
				PreparedStatement ps = conn.prepareStatement(sql)){
